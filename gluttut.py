...
class OpenGLEngine:
    def __init__(self):
        ...
        self.camera = Camera()
        self.world.set_position(0, 0, 2)
        self.mouse_picker = MouseHandler()
        self.texture = Texture('./bricks.jpeg', GL.GL_TEXTURE_2D)
        self.cube = Cube()

        if not self.texture.load():
            raise ValueError("Could not load texture")

        GL.glPointSize(15)

        self.framebuffer = Framebuffer(self.world,
                                       self.projection,
                                       self.camera,
                                       self.texture)

        self.framebuffer.add_model(self.cube)
        self.framebuffer.write_buffers()
        self.framebuffer.compile_shaders()
    ...
    def render_scene(self):
        ...
        self.mouse_picker.enable_writing()
        self.framebuffer.picking_render(self.mouse_picker.texture)
        self.mouse_picker.disable_writing()
        # picking done
        self.framebuffer.render()
    
    def rotate_yz(self, deg: int):
        rad = np.deg2rad(deg)
        self.world.set_rotation(rad, World.ROTATE_YZ)

    def rotate_xz(self, deg: int):
        rad = np.deg2rad(deg)
        self.world.set_rotation(rad, World.ROTATE_XZ)

    def rotate_xy(self, deg: int):
        rad = np.deg2rad(deg)
        self.world.set_rotation(rad, World.ROTATE_XY)
...
#     width = G    GLUT.glutInit(sys.argv)
# oordinates are inversed in opengl,/
...
#         if mouse_mode == 'Dragging' and selected_vertex: // להזיז קודקוד
...
#             cube.move_vertex_in_vector(selected_vertex, *delta[:3])..עד כא
#             framebuffer.write_buffers()// עד כאן הזזה של קודקדוד כולל 
...
if __name__ == '__main__':
    ...
    engine = OpenGLEngine()
    GLUT.glutDisplayFunc(engine.render_scene)
    GLUT.glutMainLoop()
