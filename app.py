...
def main(mouse_press, mouse_move, mouse_scroll):
    ...
    # FPS Measurement - tracks your PC's hardware state and displays this information as an overlay in-game
    ...
    while True:
        ...
        if key == 27:  # ESC
            break
        #return number or mode that the user return
...
def select_mode(key, mode):
    number = -1
    # return number if between 0 t0 9
    if 48 <= key <= 57:  # 0 ~ 9
        number = key - 48
    #return num -1 and mode 1 for
    if key == 110:  # n
        mode = 0
    # return num -1 and mode 2 for save new data
    if key == 107:  # k
        mode = 1
    return number, mode
...
def cv2glet(image):
    ...
    pyglet_image = pyglet.image.ImageData(width=cols,
                                          height=rows,
                                          format='BGR',
                                          data=raw_img,
